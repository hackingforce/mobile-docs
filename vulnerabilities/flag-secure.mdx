...
Outra abordagem é utilizar o comando `adb` para realizar uma captura de tela diretamente do dispositivo. Se a captura de tela resultar em um arquivo vazio ou inutilizável (como 0 bytes), isso é um forte indicativo de que a `FLAG_SECURE` está sendo aplicada para proteger o conteúdo da tela.
